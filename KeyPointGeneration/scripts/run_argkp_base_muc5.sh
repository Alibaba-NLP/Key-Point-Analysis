...
WORKSPACE_DIR=/path/to/KPA
DATASET_NAME=ArgKP
MAX_USED_COUNT=5
CKP_DIR=/home2/xli/KPA/KeyPointGeneration/outputs/"$DATASET_NAME"_base_MAX_USED_COUNT_"$MAX_USED_COUNT"
...
cd $WORKSPACE_DIR/KeyPointGeneration && python ft_flant5.py \
  ...
  --model_path /home2/xli/models/flan-t5-base/ \
  --output_dir $CKP_DIR \
  --batch_size 16 \
...
mkdir -p $WORKSPACE_DIR/GraphPartitioning/eval_outputs/"$DATASET_NAME"_base_MAX_USED_COUNT_"$MAX_USED_COUNT"
cd $WORKSPACE_DIR/GraphPartitioning && python eval_batch.py \
  ...
  --output_dir $WORKSPACE_DIR/GraphPartitioning/eval_outputs/"$DATASET_NAME"_base_MAX_USED_COUNT_"$MAX_USED_COUNT"
